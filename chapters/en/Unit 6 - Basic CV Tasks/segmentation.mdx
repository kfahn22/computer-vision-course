...
The intuition behind this task is _that it can be viewed as a classification for each pixel of the image_.
...
- **Panoptic Segmentation** A hybrid approach that combines elements of semantic and instance segmentation. It assigns a class and
  an instance to each pixel, effectively integrating the _what_ and _where_ aspects of the image.
...
An example is _[Segment anything model (SAM)](https://arxiv.org/abs/2304.02643)_ that is a popular prompt based model introduced
in April 2023 by _Meta AI Research, FAIR_. The model is based on the Vision Transformer (ViT) model and focuses on creating a promptable
